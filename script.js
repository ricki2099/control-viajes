document.addEventListener('DOMContentLoaded', () => {
    ...
    budgetForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const tripId = parseInt(document.getElementById('trip-select-budget').value);
        const trip = trips.find(t => t.id === tripId);
        if (trip) {
            trip.totalBudget = parseFloat(document.getElementById('total-budget').value);
    ...
    categoryBudgetForm.addEventListener('submit', (e) => {
        ...
        if (trip) {
            ...
            const amount = parseFloat(document.getElementById('category-budget').value);
            const totalCategoryBudget = Object.values(trip.categoryBudgets).reduce((sum, val) => sum + val, 0);

            if (totalCategoryBudget + amount > trip.totalBudget) {
                alert('The sum of category budgets cannot exceed the total budget.');
            } else {
    ...
    expenseForm.addEventListener('submit', (e) => {
        e.preventDefault();
        const tripId = parseInt(document.getElementById('trip-select-expense').value);
        const trip = trips.find(t => t.id === tripId);
        if (trip) {
            const newExpense = {
    ...
    filterBtn.addEventListener('click', () => {
        const tripId = parseInt(document.getElementById('trip-select-summary').value);
        const trip = trips.find(t => t.id === tripId);
        if (trip) {
    ...
    function updateTripSelects() {
        tripSelects.forEach(select => {
            select.innerHTML = trips.map(trip => `<option value="${trip.id}">${trip.destination}</option>`).join('');
        });
        if (trips.length > 0) {
    ...
    function displayCategoryBudgets(trip) {
        ...
        display.innerHTML = '<h4>Category Budgets:</h4>' +
            Object.entries(trip.categoryBudgets).map(([cat, amt]) => `<p>${cat}: ${amt.toFixed(2)} ${trip.currency}</p>`).join('');
    }
    ...
    function updateSummary(tripId) {
        ...
        summaryDisplay.innerHTML = `
            <h3>${trip.destination} Summary</h3>
            <p><strong>Total Budget:</strong> ${trip.totalBudget.toFixed(2)} ${trip.currency}</p>
            <p><strong>Total Expenses:</strong> ${totalExpenses.toFixed(2)} ${trip.currency}</p>
            <p><strong>Remaining Budget:</strong> ${remainingBudget.toFixed(2)} ${trip.currency}</p>
        `;
        ...
        if (percentageSpent >= 100) {
            alertsDisplay.innerHTML += `<div class="alert red">Alert: You have exceeded your budget!</div>`;
        } else if (percentageSpent >= 80) {
            alertsDisplay.innerHTML += `<div class="alert orange">Alert: You have spent 80% or more of your budget.</div>`;
        } else if (percentageSpent >= 50) {
            alertsDisplay.innerHTML += `<div class="alert yellow">Alert: You have spent 50% or more of your budget.</div>`;
        }
    ...
    function displayFilteredExpenses(expenses, currency) {
        ...
        display.innerHTML = '<h4>Expenses:</h4>' +
            expenses.map(exp => `<p>${exp.date} - ${exp.category}: ${exp.amount.toFixed(2)} ${currency}</p>`).join('');
    }
});